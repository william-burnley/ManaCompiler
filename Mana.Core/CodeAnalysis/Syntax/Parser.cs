    ...
    internal sealed class Parser
    {
        ...
        public Parser(string text)
        {
            ...
            do
            {
                ...
                if (token.Kind != SyntaxKind.WhiteSpace && token.Kind != SyntaxKind.InvalidToken)
                {
                    tokens.Add(token);
                }
        ...
        private ExpressionSyntax ParseAssignmentExpression()
        {
            if (Peek(0).Kind == SyntaxKind.Identifier &&
                Peek(1).Kind == SyntaxKind.Equality)
            {
        ...
        private ExpressionSyntax ParsePrimaryExpression()
        {
            switch (Current.Kind)
            {
                case SyntaxKind.OpenParenthesis:
                    ...
                    return new ParenthesisizedExpressionSyntax(left, expression, right);
